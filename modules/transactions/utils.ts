...
export function aggregateByDayOfMonth(
  transactions: Transaction[],
): Record<number, number> {
  return transactions
    .filter((trx) => trx.type === TransactionType.EXPENSE)
...
export function aggregateByType(
  ...
  return transactions.reduce<Record<TransactionType, number>>(
    (acc, transaction) => {
...
export function sumTransactionsByTitle(
  transactions: Transaction[],
...
export function countTransactionsByTitle(
  transactions: Transaction[],
): Record<string, number> {
  return transactions.reduce<Record<string, number>>((acc, transaction) => {
    const title = transaction.title;
...
export function aggregateUncategorizedTransactionsByTitle(
  transactions: Transaction[],
): Record<string, number> {
  const uncategorizedTransactions = transactions.filter(
    (trx) => trx.categoryId === StandardTransactionCategory.UNKNOWN,
  );
