...
import { useSaveTransactions } from "~/transactions/hooks";
import { csvToTransactions } from "~/transactions/transformers";
import { Transaction } from "~/transactions/types";
import { aggregateUncategorizedTransactionsByTitle } from "~/transactions/utils";

type UseTransactionsModalStateInput = {
  account: Account;
};

export const useTransactionsModalState = ({ account }: UseTransactionsModalStateInput) => {
  ...
  const onSaveTransactions = useCallback(async () => {
    ...
    onCloseTransactionsModal();
  }, [saveTransactions, onCloseTransactionsModal, state]);
  ...
  const loading = loadingTransactions || state.loadingImport;
  const error = (errorTransactions || state.importError) as Maybe<Error>;
